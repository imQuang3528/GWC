...
import { CardProps, CardRef, ConfigCardProps } from "../card/header";

export type CardCarouselRef = {
  /**
   * Close the currently revealed card
   */
  closeRevealedCard: () => void;
};
...
export type CardCarouselProps = {
  ...
  /**
   * Callback run when the first animation is finished
   */
  onFirstAnimationFinish?: () => void;
  ...
  /**
   * Duration in seconds of the mixing animation
   */
  mixingDuration: number;

  /**
   * Distance in pixels of the cards to overshoot when the mixing animation ends
   */
  mixingOvershoot: number;
  ...
  /**
   * Callback run to decide if card will be revealed
   *
   * @returns boolean whether the card will be revealed. default to true
   */
  shouldCardReveal?: () => boolean;

  /**
   * Callback run when the card is starting to be revealed
   */
  onCardRevealStart?: () => void;

  /**
   * Callback run when the card is flipped back into the place
   */
  onCardRevealEnd?: () => void;

  /**
   * Callback run when finishing the card reveal (including reshuffling animation)
   */
  onCardRevealFinish?: () => void;

  /**
   * Display a backdrop behind the revealed card
   *
   * @default true
   */
  revealBackdrop?: boolean;
   ...
   */
  cardContents?: CardProps["frontContent"][];

  /**
   * Array of cards back to be randomly chosen for each card
   */
  cardBacks?: CardProps["backImage"][];
   ...
   */
  getRevealedCardContent?: () => Promise<CardProps["frontContent"]>;
 ...
 */
export const TO_SHUFFLING_DURATION = 250; // ms

/**
 * Delay between transition to shuffling animations
 */
export const TO_SHUFFLING_DELAY = 50; // ms

/**
 * Transition time from other animation to dealing animation
 */
export const TO_DEALING_DURATION = 375; // ms
