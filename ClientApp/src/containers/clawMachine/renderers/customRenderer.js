import { getVelocity } from "matter-js/src/body/Body";
import {
  allBodies as _allBodies,
  allComposites,
  allConstraints as _allConstraints,
} from "matter-js/src/body/Composite";
import {
  clamp,
  extend,
  isArray,
  isElement,
  now,
} from "matter-js/src/core/Common";
import { trigger } from "matter-js/src/core/Events";
import { setOffset, setScale } from "matter-js/src/core/Mouse";
import { contains, overlaps } from "matter-js/src/geometry/Bounds";
import { add, normalise, perp, sub } from "matter-js/src/geometry/Vector";

/**
 * The `CustomRender` module is a simple canvas based renderer for visualising instances of `Matter.Engine`.
...
(function () {
  ...
  CustomRender._goodFps = 30;
  CustomRender._goodDelta = 1000 / 60;
  ...
  CustomRender.create = function (options) {
    ...
    var render = extend(defaults, options);
    ...
    render.controller = CustomRender;
  ...
  CustomRender.run = function (render) {
    (function loop(time) {
      ...
      CustomRender.world(render, time);

      if (render.options.showStats || render.options.showDebug) {
        CustomRender.stats(render, render.context, time);
      }

      if (render.options.showPerformance || render.options.showDebug) {
        CustomRender.performance(render, render.context, time);
      }
    })();
  };
   ...
   * @param {render} render
   */
  CustomRender.world = function (render, time) {
    var startTime = now(),
    ...
    var allBodies = _allBodies(world),
      allConstraints = _allConstraints(world),
    ...
    trigger(render, "beginRender", event);
    ...
    if (options.hasBounds) {
      ...
      for (i = 0; i < allBodies.length; i++) {
        ...
        if (overlaps(body.bounds, render.bounds)) bodies.push(body);
      }
      ...
      for (i = 0; i < allConstraints.length; i++) {
        ...
        if (
          ...
          contains(render.bounds, pointBWorld)
        )
    ...
    if (options.showBounds) CustomRender.bodyBounds(render, bodies, context);
    ...
    if (options.showIds) CustomRender.bodyIds(render, bodies, context);
    ...
    timing.lastElapsed = now() - startTime;
  };
   ...
   * @param {Number} time
   */
  CustomRender.stats = function (render, context, time) {
    var engine = render.engine,
      world = engine.world,
      bodies = _allBodies(world),
      parts = 0,
      width = 55,
      height = 44,
      x = 0,
    ...
    var sections = {
      Part: parts,
      Body: bodies.length,
      Cons: _allConstraints(world).length,
   ...
   * @param {RenderingContext} context
   */
  CustomRender.performance = function (render, context) {
    var engine = render.engine,
      timing = render.timing,
    ...
    CustomRender.status(
      ...
      CustomRender._goodDelta / lastEngineDelta,
      function (i) {
    ...
    CustomRender.status(
      ...
      1 - engineElapsedMean / CustomRender._goodFps,
      function (i) {
    ...
    CustomRender.status(
      ...
      1 - elapsedMean / CustomRender._goodFps,
      function (i) {
  ...
  CustomRender.status = function (
    ...
    for (var i = 0; i < width; i += 1) {
      context.lineTo(
        x + i,
        y + 7 - (i < count ? height * clamp(0.4 * plotY(i), -2, 2) : 0)
      );
    }
    context.stroke();
    ...
    // indicator
    context.fillStyle =
      "hsl(" + clamp(25 + 95 * indicator, 0, 120) + ",100%,60%)";
  ...
  var _updateTiming = function (render, time) {
    ...
    timing.delta = time - timing.lastTime || CustomRender._goodDelta;
  ...
  var _applyBackground = function (render, background) {
    ...
    if (/(jpg|gif|png)$|^data:/.test(background))
